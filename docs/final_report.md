...
**Deployed URL:** http://138.68.88.189  
**Swagger/OpenAPI:** http://138.68.88.189/docs  
**Public API Example:** http://138.68.88.189/api/restaurants  
**Grafana:** http://138.68.88.189:3000/login  
**Prometheus:** http://138.68.88.189:9090  
**Loki Ready:** http://138.68.88.189:3100/ready 
**Date:** May 2026  
...
|---|---|---|
| Miraxmedova Samira | U2310169 | Frontend + Report |
| Maxsetov Nurmukhamad | U2310163 | Backend Core |
| Maxsetov Salamat | U2310164 | Database + Cache |
| Mirjalilov Mirkamol | U2310171 | Docker Infrastructure + Observability |
...
Redis is used as an implemented additional data store and cache layer. The purpose of Redis is to reduce repeated database reads for frequently accessed data and to support short-lived state.

In the current implementation, Redis caching is used for the restaurant list and restaurant menu endpoints. The backend first checks Redis before reading from PostgreSQL. If cached data exists, the backend returns the cached JSON response. If the cache is empty, the backend queries PostgreSQL and stores the result in Redis with a time-to-live value.

Implemented cache targets include:
...
| Key Pattern | Purpose |
|---|---|
| `restaurants:list` | Cached list of active restaurants. |
| `restaurants:{restaurant_id}:menu` | Cached menu items for a selected restaurant. |
...
Despite these limitations, the data layer demonstrates relational database design, schema migration, seed data, constraints, indexes, and implemented Redis caching for frequently accessed restaurant and menu data.
...
Despite these limitations, the data layer demonstrates relational database design, schema migration, seed data, constraints, indexes, and implemented Redis caching.
...
| Shared state across replicas | Move token bucket state to Redis for fully distributed rate limiting. |
...
| Per-route limits | Configure different limits for public, authenticated, and admin endpoints. |
...
The current implementation demonstrates the Token Bucket algorithm inside the backend and returns HTTP 429 Too Many Requests when the request limit is exceeded. Future improvements would make the limiter fully distributed by storing token bucket state in Redis and adding per-route configuration.
...
## 9.0 DigitalOcean VPS Deployment

The project was deployed on a DigitalOcean VPS to demonstrate public access outside the local development environment. The repository was cloned on the server, environment variables were configured, and the full container stack was started with Docker Compose.

Public deployment URLs:

| Service | URL |
|---|---|
| Frontend | http://138.68.88.189 |
| Swagger / OpenAPI | http://138.68.88.189/docs |
| Restaurant API | http://138.68.88.189/api/restaurants |
| Grafana | http://138.68.88.189:3000/login |
| Prometheus | http://138.68.88.189:9090 |
| Loki readiness endpoint | http://138.68.88.189:3100/ready |

The VPS deployment ran the same main services as the local Docker Compose environment: Nginx gateway, two FastAPI backend replicas, React frontend, PostgreSQL, Redis, Prometheus, Grafana, Loki, and Promtail.
...
| Maxsetov Nurmukhamad | Backend Core | FastAPI backend, REST API, WebSocket, authentication, Redis cache, and rate limiting | Implemented backend API endpoints, JWT authentication, WebSocket backend logic, order status updates, Redis cache integration, and Token Bucket Rate Limiter. |
| Maxsetov Salamat | Database + Cache | PostgreSQL schema, Alembic migrations, seed data, indexes, and Redis cache design | Designed and implemented database schema, relationships, constraints, Alembic migrations, seed data, performance indexes, query optimization results, and Redis cache design. |
| Mirjalilov Mirkamol | Docker Infrastructure + Observability | Docker Compose, Nginx gateway, backend replicas, observability stack, and VPS deployment | Configured Docker Compose, Nginx API Gateway, backend replicas, PostgreSQL, Redis, frontend container, Prometheus, Grafana, Loki, Promtail, and DigitalOcean VPS deployment. |
